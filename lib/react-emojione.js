...
import { unicodes, unicodeToCodepoint } from './utils/emoji-format-conversion';
...
ASCII_DATA.forEach(([reStr, unicode]) => asciiRegExpToUnicode.set(RegExp(reStr), unicode)); // Escape RegExp code borrowed from lodash

const reRegExpChar = /[\\^$.*+?()[\]{}|]/g;
const reHasRegExpChar = RegExp(reRegExpChar.source);

const escapeRegExp = s => s && reHasRegExpChar.test(s) ? s.replace(reRegExpChar, '\\$&') : s;
...
const getRegex = (withUnicode, withAscii) => {
  const index = (withUnicode ? 1 : 0) + (withAscii ? 2 : 0);

  if (!REGEX_CACHE[index]) {
    const parts = [withUnicode ? unicodesRegexStr : '', withAscii ? asciiRegexStr : ''].filter(Boolean);
...
export const emojify = (str, options = {}) => {
  ...
  const regExp = getRegex(convertUnicode, convertAscii);
...
class Emojify extends React.Component {
  ...
  render() {
    ...
    return React.Children.count(children) ? React.createElement('span', {}, this.traverse(children, this.props)) : null;
  }
